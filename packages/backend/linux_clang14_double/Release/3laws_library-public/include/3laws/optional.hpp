/**
 * @file optional.hpp
 * @author Written by Sy Brand (tartanllama@gmail.com), modified by Thomas Gurriet
 * (tgurriet@3laws.io)
 * @brief  C++11 implementation of std::optional with extensions. Modified version of
 * https://github.com/TartanLlama/optional.git
 * @par License
 * Distributed under the Creative Commons Public domain license (CC0), see:
 * http://creativecommons.org/publicdomain/zero/1.0/
 * @copyright Copyright 2017 Sy Brand
 */
...
struct is_trivially_copy_constructible<std::vector<T, A>>  // NOLINT
    : std::is_trivially_copy_constructible<T>              // NOLINT
{};
#endif
}  // namespace detail
}  // namespace lll
...
#define TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) \
  lll::detail::is_trivially_copy_constructible<T>::value
...
#if defined(_MSC_VER) && _MSC_VER <= 1900
// TODO(tl): make a version which works with MSVC 2015
...
std::true_type can_swap(int) noexcept(                        // NOLINT
  noexcept(swap(std::declval<T &>(), std::declval<U &>())));  // NOLINT
...
// Trait for checking if a type is a lll::optional
template<class T>
...
struct returns_void_impl<F, optional_details::void_t<invoke_result_t<F, U...>>, U...>
...
template<class T,
  bool = TL_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)
      && TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) && TL_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
                 ...
                 private detail::optional_delete_assign_base<T>
{
  ...
  static_assert(
    !std::is_same<T, in_place_t>::value, "instantiation of optional with in_place_t is ill-formed");
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  template<class F>
  constexpr decltype(optional_map_impl(std::declval<const optional &>(), std::declval<F &&>())) map(
    F && f) const &
  {
    return optional_map_impl(*this, std::forward<F>(f));
  }
  ...
  constexpr optional(U && u) : base(in_place, std::forward<U>(u))  // NOLINT
  {}
  ...
  // TODO(tl): check exception guarantee
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  constexpr optional(nullopt_t) noexcept : m_value(nullptr) {}  // NOLINT
  ...
  constexpr optional(U && u) noexcept : m_value(std::addressof(u))  // NOLINT
  {
