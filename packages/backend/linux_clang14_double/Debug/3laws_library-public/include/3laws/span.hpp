/**
 * @file span.hpp
 * @author Written by Tristan Brindle, modified by Thomas Gurriet (tgurriet@3laws.io)
 * @brief C++11 implementation of std::span. Modified version of
 * https://github.com/tcbrindle/span.git
 * @par License
 * Distributed under the Boost Software License, Version 1.0, see:
 * https://github.com/tcbrindle/span/blob/master/LICENSE_1_0.txt
 * @copyright Copyright 2018 Tristan Brindle
 */

#ifndef THREELAWS_SPAN_HPP
#define THREELAWS_SPAN_HPP
...
struct has_size_and_data<T,
  span_details::void_t<decltype(detail::size(std::declval<T>())),
...
struct is_container_element_type_compatible<T,
  E,
  typename std::enable_if<
    ...
    && std::is_convertible<remove_pointer_t<decltype(detail::data(std::declval<T>()))> (*)[],
      E (*)[]>::value>::type> : std::true_type  // NOLINT
{};
...
class span
{
  ...
  static_assert(detail::is_complete<ElementType>::value,
    ...
    "declaration)");
  static_assert(
    !std::is_abstract<ElementType>::value, "A span's ElementType cannot be an abstract class type");
...
public:
  ...
  TCB_SPAN_CONSTEXPR11 span(pointer first_elem, pointer last_elem)
      : storage_(first_elem, static_cast<std::size_t>(std::distance(first_elem, last_elem)))
  {
    TCB_SPAN_EXPECT(
  ...
  template<typename Container,
    ...
    typename std::enable_if<
      ...
      int>::type = 0>
  constexpr span(Container & cont) : storage_(detail::data(cont), detail::size(cont))  // NOLINT
  {}
  ...
  constexpr span(const Container & cont)                  // NOLINT
      : storage_(detail::data(cont), detail::size(cont))  // NOLINT
  {}
  ...
  template<typename OtherElementType,
    ...
    typename std::enable_if<
      (Extent == dynamic_extent || OtherExtent == dynamic_extent || Extent == OtherExtent)
        && std::is_convertible<OtherElementType (*)[], ElementType (*)[]>::value,  // NOLINT
      int>::type = 0>
