...
struct QpInputFilterParams
{
  ...
  struct SolverParams
  {
    ...
    lll::optional<size_t> max_iter = {};     ///< Max number of iterations (default no limit)
    lll::optional<std::chrono::nanoseconds> max_time = {};  ///< Max solving time (default no limit)
    size_t stop_check_iter = 25;     ///< Iterations between checking stopping criterion
    bool polish            = true;   ///< Run solution polishing (uses dynamic memory)
    size_t polish_iter     = 5;      ///< Number of iterations to refine polish
